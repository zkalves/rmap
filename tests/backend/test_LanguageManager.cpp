...
private slots:
  ...
  void testTranslationsDirectoryDiscovery();
};
...
void TestLanguageManager::testEdgeCasesAndFallbacks() {
  ...
  // 8. Switching from one non-English language to another (replaces active
  // translator)
  QVERIFY(lm.setLanguage("es"));
  QCOMPARE(lm.currentLanguage(), QString("es"));
  QVERIFY(lm.setLanguage("de"));
  ...
  // 10. Switch language with nonexistent language code/name (returns false,
  // falls back)
  QVERIFY(!lm.setLanguage("nonexistent_unknown_xyz"));

  // 11. JsonTranslator with context-only translations and missing
  // name/nativeName metadata
  {
...
void TestLanguageManager::testTranslationsDirectoryDiscovery() {
  LanguageManager &lm = LanguageManager::instance();

  QTemporaryDir tempTransDir;
  QVERIFY(tempTransDir.isValid());

  // Create a custom translation file in the temporary directory
  QString customJsonPath = tempTransDir.path() + "/rmap_customtest.json";
  QFile f(customJsonPath);
  QVERIFY(f.open(QIODevice::WriteOnly | QIODevice::Text));

  QJsonObject root;
  root["code"] = "customtest";
  root["name"] = "CustomTestLanguage";
  root["nativeName"] = "CustomNative";

  QJsonObject trans;
  trans["Save"] = "CustomSave";
  root["translations"] = trans;

  f.write(QJsonDocument(root).toJson());
  f.close();

  // Verify language is not present before environment variable is set
  lm.rescanLanguages();
  QVERIFY(!lm.hasLanguage("customtest"));

  // Set RMAP_TRANSLATIONS_PATH and rescan
  qputenv("RMAP_TRANSLATIONS_PATH", tempTransDir.path().toLocal8Bit());
  lm.rescanLanguages();

  QVERIFY(lm.hasLanguage("customtest"));
  QVERIFY(lm.hasLanguage("CustomTestLanguage"));
  QVERIFY(lm.hasLanguage("CustomNative"));

  LanguageInfo info = lm.languageInfo("customtest");
  QCOMPARE(info.code, QString("customtest"));
  QCOMPARE(info.name, QString("CustomTestLanguage"));
  QCOMPARE(info.nativeName, QString("CustomNative"));
  QVERIFY(!info.isBuiltIn);

  // Switch to custom language and verify translation
  QVERIFY(lm.setLanguage("customtest"));
  QCOMPARE(lm.currentLanguage(), QString("customtest"));

  // Unset environment variable, rescan and verify cleanup
