...
void TestPathUtils::testExpandEnvVarsTilde() {
  ...
  QCOMPARE(PathUtils::expandEnvVars("~/my_folder/file.txt"),
           home + "/my_folder/file.txt");
  QCOMPARE(PathUtils::expandEnvVars("~\\windows_style\\path.txt"),
           home + "/windows_style/path.txt");
}

void TestPathUtils::testExpandEnvVarsPosix() {
  // $VAR syntax
  QCOMPARE(PathUtils::expandEnvVars("$RMAP_TEST_ROOT/templates"),
           QString("/tmp/rmap_test_root/templates"));

  // ${VAR} syntax
  QCOMPARE(
      PathUtils::expandEnvVars("${RMAP_TEST_ROOT}/${RMAP_TEST_SUB}/file.txt"),
      QString("/tmp/rmap_test_root/sub_dir/file.txt"));

  // Undefined variable expands to empty string
  QCOMPARE(PathUtils::expandEnvVars("$RMAP_NONEXISTENT_VAR_12345/tail.txt"),
           QString("/tail.txt"));
}

void TestPathUtils::testExpandEnvVarsWindows() {
  // %VAR% syntax
  QCOMPARE(PathUtils::expandEnvVars("%RMAP_TEST_ROOT%/test/%RMAP_TEST_FILE%"),
           QString("/tmp/rmap_test_root/test/output.sv"));
}

void TestPathUtils::testExpandEnvVarsMixed() {
  QCOMPARE(PathUtils::expandEnvVars(
               "$RMAP_TEST_ROOT/%RMAP_TEST_SUB%/${RMAP_TEST_FILE}"),
           QString("/tmp/rmap_test_root/sub_dir/output.sv"));
}
...
void TestPathUtils::testToRelativePath() {
  ...
  QCOMPARE(PathUtils::toRelativePath("./templates/c/reg_map.h.inja", baseDir),
           QString("./templates/c/reg_map.h.inja"));
  QCOMPARE(PathUtils::toRelativePath("templates/c/reg_map.h.inja", baseDir),
           QString("templates/c/reg_map.h.inja"));

  // Environment variables are left untouched
  QCOMPARE(PathUtils::toRelativePath("$PROJECT_ROOT/templates/t.inja", baseDir),
           QString("$PROJECT_ROOT/templates/t.inja"));
  QCOMPARE(
      PathUtils::toRelativePath("%PROJECT_ROOT%/templates/t.inja", baseDir),
      QString("%PROJECT_ROOT%/templates/t.inja"));
...
void TestPathUtils::testDefaultPathConstants() {
  ...
  QCOMPARE(PathUtils::DEFAULT_TRANSLATIONS_DIR, "./translations");
  QCOMPARE(PathUtils::defaultOutputDir(), QString("./work"));
  QCOMPARE(PathUtils::defaultTemplatesDir(), QString("./templates"));
  QCOMPARE(PathUtils::defaultTranslationsDir(), QString("./translations"));
}
...
void TestPathUtils::testDefaultDirectoryDiscovery() {
  ...
  QCOMPARE(PathUtils::defaultTemplatesDir(),
           QString("/custom/installed/templates"));
  qunsetenv("RMAP_TEMPLATES_DIR");

  qputenv("RMAP_EXAMPLES_DIR", "/custom/installed/examples");
  QCOMPARE(PathUtils::defaultExamplesDir(),
           QString("/custom/installed/examples"));
  ...
  qputenv("RMAP_TRANSLATIONS_PATH", "/custom/installed/translations");
  QCOMPARE(PathUtils::defaultTranslationsDir(),
           QString("/custom/installed/translations"));
  qunsetenv("RMAP_TRANSLATIONS_PATH");

  qputenv("RMAP_TRANSLATION_DIR", "/custom/installed/translations_alt");
  QCOMPARE(PathUtils::defaultTranslationsDir(),
           QString("/custom/installed/translations_alt"));
  qunsetenv("RMAP_TRANSLATION_DIR");
}

void TestPathUtils::testResolvePathWithTemplatesSubdir() {
  // When secondary base dir is a templates folder (e.g.
  // /opt/share/rmap/templates) and path starts with
  // "templates/c/reg_map.h.inja", it should resolve correctly to
  // /opt/share/rmap/templates/c/reg_map.h.inja (stripping redundant
  // "templates/" prefix)
  ...
  QString resolved =
      PathUtils::resolvePath("templates/c/reg_map.h.inja", "", shareTmplDir);
  QVERIFY(QFile::exists(resolved));
  QCOMPARE(resolved,
           PathUtils::normalizeSeparators(shareTmplDir + "/c/reg_map.h.inja"));

  QString resolvedRel =
      PathUtils::resolvePath("./templates/c/reg_map.h.inja", "", shareTmplDir);
  QVERIFY(QFile::exists(resolvedRel));
  QCOMPARE(resolvedRel,
           PathUtils::normalizeSeparators(shareTmplDir + "/c/reg_map.h.inja"));

  // Non-existent template targets falling back to constructed path in
  // secondaryBaseDir
  QString nonEx1 =
      PathUtils::resolvePath("templates/non_existent.inja", "", shareTmplDir);
  QCOMPARE(nonEx1,
           PathUtils::normalizeSeparators(shareTmplDir + "/non_existent.inja"));
  ...
  QString nonEx3 =
      PathUtils::resolvePath("non_existent.inja", "", shareTmplDir);
  QCOMPARE(nonEx3,
           PathUtils::normalizeSeparators(shareTmplDir + "/non_existent.inja"));

  // PrimaryBaseDir ending with /templates where path does not start with
  // templates/ and doesn't exist
  QString nonExP = PathUtils::resolvePath("not_found.txt", shareTmplDir, "");
  QCOMPARE(nonExP,
           PathUtils::normalizeSeparators(shareTmplDir + "/not_found.txt"));
}

void TestPathUtils::testEdgeCasesAndOverloads() {
  // UNC paths
  QCOMPARE(PathUtils::normalizeSeparators("//server/share/file.txt"),
           QString("//server/share/file.txt"));
  QCOMPARE(PathUtils::normalizeSeparators("///server/share/file.txt"),
           QString("/server/share/file.txt"));
  ...
  QString absBase =
      QFileInfo("work/test_path_utils/base_file.txt").absoluteFilePath();
  QString absTarget =
      QFileInfo("work/test_path_utils/target.txt").absoluteFilePath();
  ...
  QCOMPARE(cwdFallback, PathUtils::normalizeSeparators(QDir::current().filePath(
                            "nonexistent_target_123.txt")));

  // resolvePath fallback with secondary base dir ending with templates
  QString shareTmplDir = QDir("work/test_path_utils/templates").absolutePath();
  QString tmplFallback =
      PathUtils::resolvePath("templates/sub/new_file.txt", "", shareTmplDir);
  QCOMPARE(tmplFallback,
           PathUtils::normalizeSeparators(shareTmplDir + "/sub/new_file.txt"));
  QString tmplFallback2 =
      PathUtils::resolvePath("./templates/sub/new_file2.txt", "", shareTmplDir);
  QCOMPARE(tmplFallback2,
           PathUtils::normalizeSeparators(shareTmplDir + "/sub/new_file2.txt"));
  QString expectedTmplBase = QFileInfo("templates").absoluteFilePath();
  QString tmplFallback3 =
      PathUtils::resolvePath("templates/sub/new_file3.txt", "", "templates");
  QCOMPARE(tmplFallback3,
           PathUtils::normalizeSeparators(
               QDir(expectedTmplBase).filePath("sub/new_file3.txt")));
  QString tmplFallback4 =
      PathUtils::resolvePath("./templates/sub/new_file4.txt", "", "templates");
  QCOMPARE(tmplFallback4,
           PathUtils::normalizeSeparators(
               QDir(expectedTmplBase).filePath("sub/new_file4.txt")));
  ...
  QCOMPARE(PathUtils::resolvePath(s_in, std::string(""), std::string("")),
           PathUtils::resolvePath(QString::fromStdString(s_in)).toStdString());
  ...
  QCOMPARE(PathUtils::expandEnvVars(static_cast<const char *>(nullptr)),
           QString(""));
  QCOMPARE(PathUtils::toRelativePath(static_cast<const char *>(nullptr)),
           QString(""));
  QCOMPARE(PathUtils::resolvePath(static_cast<const char *>(nullptr)),
           QString(""));
  QCOMPARE(PathUtils::normalizeSeparators(static_cast<const char *>(nullptr)),
           QString(""));
}
...
void TestPathUtils::testExtendedPathResolutionAndDiscovery() {
  ...
  // Test primaryBaseDir == "templates" (exercises normP == "templates" in
  // PathUtils::resolvePath)
  QDir().mkpath("templates/c");
  QFile fLocal("templates/c/dummy_prim.inja");
  if (fLocal.open(QIODevice::WriteOnly)) {
    fLocal.write("x");
    fLocal.close();
  }
  QString resTmplBase =
      PathUtils::resolvePath("templates/c/dummy_prim.inja", "templates");
  ...
  QString outFallback = PathUtils::resolvePath("out_nonexistent.sv", "",
                                               "/tmp/secondary_out_base");
  QCOMPARE(outFallback, QString("/tmp/secondary_out_base/out_nonexistent.sv"));

  // 3. Relocatable directory discovery (<appDir>/../share/rmap/...) and
  // relative fallback
  ...
  QString relShareTrans = appDir + "/../share/rmap/translations";

  QDir().mkpath(relShareTmpl);
  QDir().mkpath(relShareEx);
  QDir().mkpath(relShareDoc);
  QDir().mkpath(relShareTrans);
  ...
  QVERIFY(!PathUtils::defaultTranslationsDir().isEmpty());
  ...
  QCOMPARE(PathUtils::normalizeSeparators("///triple/slash/file.txt"),
           QString("/triple/slash/file.txt"));
  QCOMPARE(PathUtils::normalizeSeparators("//double/slash/file.txt"),
           QString("//double/slash/file.txt"));
  QCOMPARE(PathUtils::normalizeSeparators("/single/slash/file.txt"),
           QString("/single/slash/file.txt"));

  // 5. resolvePath with non-existent files and templates prefix variants
  QString resNonEx1 = PathUtils::resolvePath("templates/dummy_nonexistent.inja",
                                             "", "templates");
  QVERIFY(resNonEx1.endsWith("templates/dummy_nonexistent.inja"));

  QString resNonEx2 = PathUtils::resolvePath(
      "./templates/dummy_nonexistent.inja", "", "templates");
  QVERIFY(resNonEx2.endsWith("templates/dummy_nonexistent.inja"));

  QString resNonEx3 =
      PathUtils::resolvePath("other/dummy_nonexistent.inja", "", "templates");
  QVERIFY(resNonEx3.endsWith("templates/other/dummy_nonexistent.inja"));

  QString resNonEx4 =
      PathUtils::resolvePath("dummy_nonexistent.inja", "", "custom_base");
  QVERIFY(resNonEx4.endsWith("custom_base/dummy_nonexistent.inja"));

  // 6. Local examples directory detection branches: rmt subdir vs non-empty vs
  // empty
  ...
  dummyBaseFile.close();
  QString resFromFile =
      PathUtils::resolvePath("dummy_regmap.rmt", dummyBaseFile.fileName());
  QVERIFY(QFile::exists(resFromFile));

  // 9. Base directory ending with templates and subTmpl starting with
  // templates/
  QDir().mkpath("work/test_path_utils/templates/c");
  ...
  QString resSub1 = PathUtils::resolvePath("templates/c/reg_map.h.inja",
                                           "work/test_path_utils/templates");
  QVERIFY(QFile::exists(resSub1));

  QString resSub2 = PathUtils::resolvePath("templates/c/reg_map.h.inja", "",
                                           "work/test_path_utils/templates");
  QVERIFY(QFile::exists(resSub2));

  QString resSub3 = PathUtils::resolvePath("./templates/c/reg_map.h.inja", "",
                                           "work/test_path_utils/templates");
  ...
  QString resLit3 =
      PathUtils::resolvePath("templates/c/reg_map.h.inja", "", "templates");
  QVERIFY(!resLit3.isEmpty());

  // 11. Empty environment variables for default directories (tests
  // !envDir.isEmpty() false branches)
