...
void TestCodeGenerator::testHelperUpperAndLower() {
  CodeGenerator cg;
  Environment env;
  // Test helper registration via private helper callback mechanism indirectly
  // or directly
  json data;
  ...
  GenerationReport report =
      cg.generate(data, "work/test_tmpl", "work/test_tmpl", mappings);
...
void TestCodeGenerator::testHelperToHexAndToDec() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"work/test_tmpl/hex_dec.inja", "work/test_tmpl/hex_dec.txt"});
...
void TestCodeGenerator::testHelperSvHex() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"work/test_tmpl/sv_hex.inja", "work/test_tmpl/sv_hex.txt"});
...
void TestCodeGenerator::testFullGenerationUvmModel() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/uvm/reg_model.sv.inja", "work/uvm/test_uvm_model.sv"});
...
void TestCodeGenerator::testUvmCookbookRalFeatures() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/uvm/reg_model.sv.inja", "work/uvm/ral_features_model.sv"});
  ...
  QVERIFY(content.contains(
      "this.data.configure(this, 8, 0, \"RO\", true, 0x00, true, false, 0);"));

  // Verify custom map declaration and creation
  QVERIFY(content.contains("uvm_reg_map ahb_map;"));
  QVERIFY(content.contains("this.ahb_map = create_map(\"AHB_MAP\", 32'h1000, "
                           "4, UVM_LITTLE_ENDIAN, 1);"));

  // Verify register added to default_map and AHB_map
  QVERIFY(content.contains(
      "this.default_map.add_reg(this.rx_fifo, 32'h0000, \"RO\");"));
  QVERIFY(content.contains(
      "this.ahb_map.add_reg(this.rx_fifo, 32'h0000, \"RO\");"));

  // Verify custom HDL backdoor path
  QVERIFY(content.contains(
      "this.rx_fifo.add_hdl_path_slice(\"dut.soc.rx_fifo_dff\", 0, 32);"));
  ...
  QVERIFY(
      content.contains("this.sram_buf = new(\"SRAM_BUF\", 2048, 32, \"RW\");"));
  QVERIFY(
      content.contains("this.sram_buf.configure(this, \"dut.soc.sram_arr\");"));
  ...
  QVERIFY(content.contains(
      "this.gpio = gpio_reg_block::type_id::create(\"GPIO\");"));
  QVERIFY(content.contains(
      "this.default_map.add_submap(this.gpio.default_map, 32'h0800);"));
  QVERIFY(content.contains(
      "this.ahb_map.add_submap(this.gpio.ahb_map, 32'h0800);"));

  // Verify indirect register extends uvm_reg_indirect_data
  QVERIFY(
      content.contains("class mem_window_reg extends uvm_reg_indirect_data;"));
  QVERIFY(content.contains(
      "this.mem_window.configure(this.rx_fifo, null, this, null);"));
  ...
  QVERIFY(
      content.contains("covergroup top_subsystem_reg_access_cg with function "
...
void TestCodeGenerator::testMultiSourceTemplateMappings() {
  ...
  mappings.push_back(
      {"templates/uvm/reg_model.sv.inja", "work/uvm/multi_out2.sv"});

  GenerationReport report =
      cg.generate(data, "./templates", "./work", mappings);
  QVERIFY(!report.has_errors());
...
void TestCodeGenerator::testNewNamingAndTypeHelpers() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"work/test_tmpl/helpers.inja", "work/test_tmpl/helpers.txt"});
...
void TestCodeGenerator::testFullGenerationGenericRtl() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/rtl/spi_core_reg_file.sv"});
...
void TestCodeGenerator::testFullGenerationApbAndAxilWrappers() {
  ...
  mappings.push_back({"templates/rtl/apb_reg_file.sv.inja",
                      "work/rtl/spi_core_apb_reg_file.sv"});
  mappings.push_back({"templates/rtl/axil_reg_file.sv.inja",
                      "work/rtl/spi_core_axil_reg_file.sv"});
  ...
  QVERIFY(axilContent.contains(
      "input  logic [ADDR_WIDTH-1:0]     s_axil_awaddr_i"));
  QVERIFY(axilContent.contains(
      "output logic                      s_axil_awready_o"));
  QVERIFY(
      axilContent.contains("input  logic [DATA_WIDTH-1:0]     s_axil_wdata_i"));
  QVERIFY(axilContent.contains(
      "output logic                      s_axil_wready_o"));
  QVERIFY(
      axilContent.contains("output logic [1:0]                s_axil_bresp_o"));
  QVERIFY(axilContent.contains(
  ...
  QVERIFY(
      axilContent.contains("output logic [DATA_WIDTH-1:0]     s_axil_rdata_o"));
  QVERIFY(
      axilContent.contains("output logic [1:0]                s_axil_rresp_o"));
  QVERIFY(axilContent.contains(
...
void TestCodeGenerator::testFullGenerationRtlWithMemories() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/rtl/periph_reg_file.sv"});
  mappings.push_back(
      {"templates/rtl_tb/tb_reg_map.sv.inja", "work/rtl_tb/tb_periph.sv"});
  mappings.push_back(
      {"templates/uvm/reg_model.sv.inja", "work/uvm/periph_reg_model.sv"});
  ...
  QVERIFY(rtlContent.contains(
      "output logic                      mem_buffer_ram_req_o"));
  QVERIFY(rtlContent.contains(
      "output logic                      mem_buffer_ram_we_o"));
  QVERIFY(rtlContent.contains(
      "output logic [ADDR_WIDTH-1:0]     mem_buffer_ram_addr_o"));
  QVERIFY(rtlContent.contains(
      "output logic [DATA_WIDTH-1:0]     mem_buffer_ram_wdata_o"));
  QVERIFY(rtlContent.contains(
      "output logic [STRB_WIDTH-1:0]     mem_buffer_ram_wstrb_o"));
  QVERIFY(rtlContent.contains(
      "input  logic [DATA_WIDTH-1:0]     mem_buffer_ram_rdata_i"));
  QVERIFY(rtlContent.contains(
      "input  logic                      mem_buffer_ram_ready_i"));
  QVERIFY(rtlContent.contains(
      "localparam logic [ADDR_WIDTH-1:0] MEM_BUFFER_RAM_START = 32'h1000;"));
  QVERIFY(rtlContent.contains(
      "localparam logic [ADDR_WIDTH-1:0] MEM_BUFFER_RAM_SIZE  = 1024;"));
  ...
  QVERIFY(
      tbContent.contains("Phase 6: Memory Subsystem Passthrough Verification"));
  ...
  QVERIFY(uvmContent.contains(
      "this.default_map.add_mem(this.buffer_ram, 0x1000);"));
...
void TestCodeGenerator::testFullGenerationRustPac() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/rust/reg_map.rs.inja", "work/rust/spi_pac.rs"});
...
void TestCodeGenerator::testFullGenerationPythonDriver() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/python/reg_map.py.inja", "work/python/spi_driver.py"});
...
void TestCodeGenerator::testFullGenerationHtmlDoc() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/html/reg_doc.html.inja", "work/html/spi_doc.html"});
...
void TestCodeGenerator::testFullGenerationSystemRdl() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/systemrdl/reg_map.rdl.inja", "work/systemrdl/spi_map.rdl"});
...
void TestCodeGenerator::testFullGenerationIpxact() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/ipxact/reg_map.xml.inja", "work/ipxact/spi_ipxact.xml"});
...
void TestCodeGenerator::testFullGenerationMarkdownDoc() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/markdown/reg_doc.md.inja", "work/markdown/spi_doc.md"});
...
void TestCodeGenerator::testFullGenerationJsonMap() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/json/reg_map.json.inja", "work/json/spi_map.json"});
...
void TestCodeGenerator::testRelativePathAndBaseDirResolution() {
  ...
  std::string baseDir =
      QDir("work/test_codegen_rel/subproject").absolutePath().toStdString();

  std::vector<TemplateMapping> mappings;
  mappings.push_back({"./templates/rel_test.inja", "./build/rel_test.txt"});

  GenerationReport report =
      cg.generate(data, "./templates", "./build", mappings, baseDir);
  QVERIFY(!report.has_errors());
  QCOMPARE(report.success_files.size(), (size_t)1);
  ...
  globMappings.push_back(
      {"global_rel.inja", "work/test_codegen_rel/global_out.txt"});
  GenerationReport globReport =
      cg.generate(data, "work/test_codegen_rel/nonexistent_folder",
                  "work/test_codegen_rel", globMappings, "");
  ...
  // resolveOutputPath prefix branches: line 253 (./ + expDefaultTmpl + /)
  {
    ...
    data["name"] = "pfx_test";
    data["blocks"] = json::array(
        {{{"name", "ctrl"}, {"offset", 0}, {"registers", json::array()}}});
    std::vector<TemplateMapping> pfxMappings;
    pfxMappings.push_back({"./templates/c/reg_map.h.inja", ""});
    GenerationReport pfxReport = cg.generate(
        data, "templates", "work/test_codegen_rel/pfx1", pfxMappings);
    qunsetenv("RMAP_TEMPLATES_DIR");
    QVERIFY(!pfxReport.has_errors());
    QVERIFY(QFile::exists("work/test_codegen_rel/pfx1/c/reg_map.h"));
  }

  // resolveOutputPath prefix branches: line 257 (./templates/ when
  // expDefaultTmpl != templates)
  {
    ...
    GenerationReport pfx2Report = cg.generate(
        data, "templates", "work/test_codegen_rel/pfx2", pfx2Mappings);
    qunsetenv("RMAP_TEMPLATES_DIR");
...
void TestCodeGenerator::testEnvVarExpansionInTemplateAndOutput() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back({"$RMAP_CODEGEN_TMPL/env_template.inja",
                      "${RMAP_CODEGEN_OUT}/env_out.txt"});
...
void TestCodeGenerator::testDynamicPathVariableSubstitution() {
  ...
  // Test meaningful variables: {output_folder}, {category}, {block_name},
  // {file_extension}
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
  ...
  GenerationReport report =
      cg.generate(data, "templates", "work/custom_out", mappings);
  QVERIFY(!report.has_errors());
  QCOMPARE(report.success_files.size(), (size_t)1);
  QVERIFY(QFile::exists("work/custom_out/c/spi_core_custom.h"));

  // Test category directory override deduplication (hw/rtl/ does not produce
  // hw/rtl/rtl)
  std::vector<TemplateMapping> dirMappings;
  dirMappings.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/dedup_test/rtl/"});
  GenerationReport report2 =
      cg.generate(data, "templates", "work/dedup_test", dirMappings);
  ...
  // Test category directory override deduplication without trailing slash
  // (work/dedup_test2/rtl)
  std::vector<TemplateMapping> dirMappings2;
  dirMappings2.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/dedup_test2/rtl"});
  GenerationReport report3 =
      cg.generate(data, "templates", "work/dedup_test2", dirMappings2);
  ...
  nameOnlyMappings.push_back(
      {"templates/c/reg_map.h.inja", "work/name_only/{block_name}.h"});
  GenerationReport report4 = cg.generate(dataNoBlocks, "templates",
                                         "work/name_only", nameOnlyMappings);
...
void TestCodeGenerator::testRtlStrobeAndCrcGeneration() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/rtl/test_strobe.sv"});
  ...
  QVERIFY(
      content.contains("localparam logic [31:0] REGMAP_CRC32 = 32'hCAFE1234;"));
...
void TestCodeGenerator::testDynamicSimulationPathResolution() {
  ...
  mappings.push_back({"templates/rtl/reg_map.sv.inja",
                      "work/hardware/custom_rtl/spi_core.sv"});
  mappings.push_back({"templates/uvm/reg_model.sv.inja",
                      "work/verif/custom_uvm/spi_model.sv"});
  mappings.push_back(
      {"templates/sim/Makefile.inja", "work/build/sim/Makefile"});
  ...
  QVERIFY2(
      mkContent.contains(
          "RTL_SRC     ?= ../../hardware/custom_rtl/spi_core.sv"),
      qPrintable(QString("Expected RTL_SRC relative path in Makefile, got:\n%1")
                     .arg(mkContent)));
  QVERIFY2(
      mkContent.contains("UVM_SRC     ?= ../../verif/custom_uvm/spi_model.sv"),
...
void TestCodeGenerator::testPythonScriptExecutionOnGeneration() {
  ...
  // 1. Create a Python script that asserts all Inja template variables are
  // available
  ...
  out << "assert project_name == 'SPI_Controller', f'Expected project_name "
         "SPI_Controller, got {project_name}'\n";
  out << "assert project_version == '2.1.0', f'Expected project_version 2.1.0, "
         "got {project_version}'\n";
  out << "assert reg_width == 32, f'Expected reg_width 32, got {reg_width}'\n";
  out << "assert reg_width_bytes == 4, f'Expected reg_width_bytes 4, got "
         "{reg_width_bytes}'\n";
  out << "assert custom_key == 'custom_val', f'Expected custom_val, got "
         "{custom_key}'\n";
  out << "assert len(blocks) == 1, f'Expected 1 block, got {len(blocks)}'\n";
  out << "assert blocks[0]['name'] == 'SPI0', f'Expected SPI0, got "
         "{blocks[0][\"name\"]}'\n";
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/c/reg_map.h.inja", "work/test_python_exec/reg_map.h"});

  CodeGenerator cg;
  GenerationReport report =
      cg.generate(testJson, "templates", "work/test_python_exec", mappings,
                  QDir::currentPath().toStdString(), scriptPath.toStdString());

  QVERIFY2(!report.has_errors(),
           report.errors.empty() ? "" : report.errors[0].second.c_str());
  ...
  QTest::ignoreMessage(QtWarningMsg,
                       QRegularExpression(".*Python script execution failed: "
                                          "Intentional script failure test.*"));
  GenerationReport failReport = cg.generate(
      testJson, "templates", "work/test_python_exec", mappings,
      QDir::currentPath().toStdString(), failScriptPath.toStdString());

  QVERIFY(failReport.has_errors());
  QCOMPARE(failReport.errors.size(), (size_t)1);
  QVERIFY(QString::fromStdString(failReport.errors[0].second)
              .contains("Intentional script failure test"));

  // 3. Test error handling when script file does not exist
  QTest::ignoreMessage(
      QtWarningMsg, QRegularExpression(".*Python script execution failed: "
                                       "Python script file does not exist.*"));
  GenerationReport missingReport =
      cg.generate(testJson, "templates", "work/test_python_exec", mappings,
                  QDir::currentPath().toStdString(),
                  "work/test_python_exec/does_not_exist.py");
  QVERIFY(missingReport.has_errors());
  QVERIFY(QString::fromStdString(missingReport.errors[0].second)
...
void TestCodeGenerator::testErrorRecoveryAndInvalidTemplates() {
  ...
  QTest::ignoreMessage(
      QtWarningMsg, QRegularExpression(".*Template file does not exist: "
                                       "templates/non_existent_tmpl\\.inja.*"));
  GenerationReport rep =
      cg.generate(data, "templates", "work/cg_err", mappings);
  QVERIFY(rep.has_errors());
  QVERIFY(rep.errors[0].second.find("Template file does not exist") !=
          std::string::npos);
  ...
  QTest::ignoreMessage(
      QtWarningMsg,
      QRegularExpression(
          ".*Template rendering error:.*unknown function undefined_fn.*"));
  GenerationReport badRep =
      cg.generate(data, "templates", "work/cg_err", badMappings,
                  QDir::currentPath().toStdString());
  QVERIFY(badRep.has_errors());
  QVERIFY(badRep.errors[0].second.find("Template rendering error") !=
          std::string::npos);

  // 3. Empty mappings triggers parseDirectory fallback
  data["blocks"] = json::array(
      {{{"name", "ctrl_block"}, {"offset", 0}, {"registers", json::array()}}});
  GenerationReport emptyMapRep =
      cg.generate(data, "templates/c", "work/cg_empty_map", {});
...
void TestCodeGenerator::testHelpersExtendedEdgeCases() {
  ...
  tmpl.write("upper_q={{ upper(\"quoted\") }}\n"
  ...
  GenerationReport rep =
      cg.generate(data, "templates", "work/cg_helpers", mappings,
                  QDir::currentPath().toStdString());
  QVERIFY(!rep.has_errors());
...
void TestCodeGenerator::testLegacyAndDirectoryMethods() {
  ...
  GenerationReport repNonEx =
      cg.parseDirectory(data, "non_existent_folder_xyz_123", "work/cg_out");
  QVERIFY(repNonEx.has_errors());
  QVERIFY(repNonEx.errors[0].second.find("Template directory does not exist") !=
          std::string::npos);

  // 4. parseDirectory with empty directory
  QDir().mkpath("work/cg_empty_tmpl_dir");
  GenerationReport repEmpty =
      cg.parseDirectory(data, "work/cg_empty_tmpl_dir", "work/cg_out");
  ...
  dummyScript.close();
  GenerationReport repEmptyPy =
      cg.parseDirectory(data, "work/cg_empty_tmpl_dir", "work/cg_out", "",
                        dummyScript.fileName().toStdString());
  ...
  GenerationReport repTmpl =
      cg.parseDirectory(data, "work/cg_empty_tmpl_dir", "work/cg_tmpl_out");
  QVERIFY(!repTmpl.has_errors());
  QVERIFY(QFile::exists("work/cg_tmpl_out/sample"));

  // 6. Empty template path returns empty string
  std::vector<TemplateMapping> emptyTmplMapping = {
      {"", "work/cg_empty_tmpl_out.txt"}};
  ...
  padData["blocks"] = json::array(
      {{{"name", "b1"},
        {"offset", 0},
        {"registers", json::array({{{"name", "r1"}, {"offset", 0}},
                                   {{"name", "r2"}, {"offset", 0}}})}}});
  cg.generate(padData, "templates/c", "work/cg_pad_test",
              {{"templates/c/reg_map.h.inja", "work/cg_pad_test/reg_map.h"}});
}
...
void TestCodeGenerator::testPythonRunnerEdgeCases() {
  ...
  bool okNonEx =
      cg.runPythonScript("non_existent_script_xyz.py", data, "", nullptr, &err);
  QVERIFY(!okNonEx);
  ...
  bool okBadExe = cg.runPythonScript(scriptFile.fileName().toStdString(), data,
                                     "", nullptr, &err);
  QVERIFY(!okBadExe);
  ...
  bool okNoPy = cg.runPythonScript(scriptFile.fileName().toStdString(), data,
                                   "", nullptr, &err);
  qputenv("PATH", savedPath);
  QVERIFY(!okNoPy);
  QVERIFY(err.find("Python interpreter ('python3' or 'python') not found in "
                   "system PATH.") != std::string::npos);

  // 5. Temporary JSON file failure via RMAP_TMPDIR (lines 607-608)
  qputenv("RMAP_TMPDIR", "/dev/null/not_a_dir_12345");
  bool okBadTmp = cg.runPythonScript(scriptFile.fileName().toStdString(), data,
                                     "", nullptr, &err);
  ...
  bool okTimeout = cg.runPythonScript(sleepScript.fileName().toStdString(),
                                      data, "", nullptr, &err);
  ...
  // 8. Successful execution with non-existent base_dir (workDir fallback to
  // script dir)
  bool okSuccessNonExDir =
      cg.runPythonScript(scriptFile.fileName().toStdString(), data,
                         "/non_existent_folder_xyz_12345", &outStr, &err);
  QVERIFY(okSuccessNonExDir);

  // 9. Successful execution with existing base_dir
  bool okSuccessWorkDir =
      cg.runPythonScript(scriptFile.fileName().toStdString(), data,
                         "work/cg_py_test", &outStr, &err);
  ...
  bool okFail = cg.runPythonScript(failScript.fileName().toStdString(), data,
                                   "", &outStr, &err);
  QVERIFY(!okFail);
  QVERIFY(err.find("fatal script error") != std::string::npos);

  // 10b. Python script failure with empty stderr (pyErr empty, error on stdout
  // instead)
  QFile failStdoutScript("work/cg_py_test/fail_stdout.py");
  QVERIFY(failStdoutScript.open(QIODevice::WriteOnly | QIODevice::Text));
  failStdoutScript.write(
      "import sys\nsys.stdout.write('error on stdout\\n')\nsys.exit(1)\n");
  failStdoutScript.close();

  bool okFailStdout = cg.runPythonScript(
      failStdoutScript.fileName().toStdString(), data, "", &outStr, &err);
  ...
  cg.runPythonScript(scriptFile.fileName().toStdString(), data, "", nullptr,
                     nullptr);

  // 10d. RMAP_PYTHON_TIMEOUT_MS env var
  qputenv("RMAP_PYTHON_TIMEOUT_MS", "5000");
  bool okTimeoutMs = cg.runPythonScript(scriptFile.fileName().toStdString(),
                                        data, "", &outStr, &err);
  qunsetenv("RMAP_PYTHON_TIMEOUT_MS");
  QVERIFY(okTimeoutMs);

  // 11. Template categorization pre-scan mappings: rtl, rtl_tb, uvm, uvm_tb,
  // sim dir, sim file
  {
    json root;
    root["name"] = "SCAN_TEST";
    ...
    fRtlTmpl.open(QIODevice::WriteOnly);
    fRtlTmpl.write("// rtl\n");
    fRtlTmpl.close();

    QFile fRtlTbTmpl("work/scan_test/rtl_tb/tb_reg_map.sv.inja");
    fRtlTbTmpl.open(QIODevice::WriteOnly);
    fRtlTbTmpl.write("// rtl_tb\n");
    fRtlTbTmpl.close();

    QFile fUvmTmpl("work/scan_test/uvm/reg_model.sv.inja");
    fUvmTmpl.open(QIODevice::WriteOnly);
    fUvmTmpl.write("// uvm\n");
    fUvmTmpl.close();

    QFile fUvmTbTmpl("work/scan_test/uvm_tb/tb_reg_model.sv.inja");
    fUvmTbTmpl.open(QIODevice::WriteOnly);
    fUvmTbTmpl.write("// uvm_tb\n");
    fUvmTbTmpl.close();

    QFile fSimTmpl("work/scan_test/sim/Makefile.inja");
    fSimTmpl.open(QIODevice::WriteOnly);
    fSimTmpl.write(
        "RTL_REL = {{ sim_rel_rtl_path }}\nUVM_REL = {{ sim_rel_uvm_path }}\n");
    fSimTmpl.close();

    std::vector<TemplateMapping> scanMappings = {
        {"work/scan_test/rtl/reg_map.sv.inja",
         "work/scan_test/out_rtl/reg_map.sv"},
        {"work/scan_test/rtl_tb/tb_reg_map.sv.inja",
         "work/scan_test/out_rtl_tb/tb_reg_map.sv"},
        {"work/scan_test/uvm/reg_model.sv.inja",
         "work/scan_test/out_uvm/reg_model.sv"},
        {"work/scan_test/uvm_tb/tb_reg_model.sv.inja",
         "work/scan_test/out_uvm_tb/tb_reg_model.sv"},
        {"work/scan_test/sim/Makefile.inja", "work/scan_test/sim/"},
        {"work/scan_test/sim/Makefile.inja", "work/scan_test/sim/Makefile"}};

    GenerationReport scanReport =
        cg.generate(root, "work/scan_test", "work/scan_test/out", scanMappings);
...
void TestCodeGenerator::testCommandLineInterface() {
  ...
  auto runRmap = [&](const QStringList &args,
                     const QProcessEnvironment *customEnv =
                         nullptr) -> QPair<int, QString> {
    QProcess proc;
    proc.setProcessChannelMode(QProcess::MergedChannels);
    QProcessEnvironment env =
        customEnv ? *customEnv : QProcessEnvironment::systemEnvironment();
    env.insert("QT_QPA_PLATFORM", "offscreen");
    proc.setProcessEnvironment(env);
  ...
  // 9. Short and long option variants (-h, -v, --export, --convert, --lint,
  // --diff)
  {
  ...
  // 13. Additional CLI option permutations (--help, --version, -c, -d,
  // --strict, report formats)
  {
    ...
    QProcessEnvironment dispWaylandEnv =
  ...
  // 13. Interactive GUI startup and clean SIGTERM shutdown (exercises
  // mainWin->show() and app.exec())
  {
    QProcess proc;
    QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    env.insert("QT_QPA_PLATFORM", "offscreen");
  ...
  // 16. sv_hex helper with string width argument
  {
    ...
    std::vector<TemplateMapping> mappings;
    mappings.push_back(
        {"work/test_tmpl/sv_hex_str.inja", "work/test_tmpl/sv_hex_str.txt"});
    json emptyObj = json::object();
    emptyObj["name"] = "sv_hex_test";
    GenerationReport report =
        cg.generate(emptyObj, "work/test_tmpl", "work/test_tmpl", mappings);
    QVERIFY(!report.has_errors());
  ...
  // 17. generate with reg_width 0 and decreasing register offsets
  {
    ...
    std::vector<TemplateMapping> mappings;
    mappings.push_back(
        {"work/test_tmpl/pad_test.inja", "work/test_tmpl/pad_test.txt"});
    GenerationReport report =
        cg.generate(padRoot, "work/test_tmpl", "work/test_tmpl", mappings);
    QVERIFY(!report.has_errors());
  }
  ...
  // 18. runPythonScript with mismatched json types, script failure, and timeout
  {
    ...
    if (fFail.open(QIODevice::WriteOnly | QIODevice::Text)) {
      fFail.write("import sys\nsys.stderr.write('fatal script "
                  "error\\n')\nsys.exit(2)\n");
      fFail.close();
      std::string stdout_str, stderr_str;
      bool ok = cg.runPythonScript(failScript.toStdString(), badTypesJson, "",
    ...
    if (fTimeout.open(QIODevice::WriteOnly | QIODevice::Text)) {
      ...
      std::string stdout_str, stderr_str;
      bool ok = cg.runPythonScript(timeoutScript.toStdString(), json::object(),
                                   "", &stdout_str, &stderr_str);
      QVERIFY(!ok);
      qunsetenv("RMAP_PYTHON_TIMEOUT");
    }
  ...
  // 19. Sim template mappings, directory scanning with .tmpl, and python
  // failure to stdout
  {
    CodeGenerator cg;

    // .tmpl template extension stripping during directory scanning (exercises
    // line 556)
    ...
    GenerationReport rTmpl = cg.parseDirectory(
        json::object(), "work/test_tmpl_scan", "work/out_tmpl_scan");
    Q_UNUSED(rTmpl);

    // Python script that fails and outputs to stdout only (empty stderr ->
    // exercises pyErr.empty() ? pyOut : pyErr)
    ...
    GenerationReport r = cg.generate(json::object(), "templates", "work", {},
                                     "", failStdoutScript.toStdString());
    QVERIFY(r.has_errors());

    // Generate with RTL, UVM, and SIM Makefile templates together (exercises
    // lines 433, 443, 451)
    std::vector<TemplateMapping> simMappings;
    simMappings.push_back(
        {"templates/rtl/reg_map.sv.inja", "work/sim_test/rtl/reg_map.sv"});
    simMappings.push_back(
        {"templates/uvm/reg_model.sv.inja", "work/sim_test/uvm/reg_model.sv"});
    simMappings.push_back(
        {"templates/sim/Makefile.inja", "work/sim_test/sim/Makefile"});
    ...
    GenerationReport simRep =
        cg.generate(simJson, "templates", "work/sim_test", simMappings);
    ...
    simDirMappings.push_back(
        {"templates/sim/Makefile.inja", "work/sim_test/sim_dir/"});
    GenerationReport simDirRep =
        cg.generate(simJson, "templates", "work/sim_test", simDirMappings);
...
void TestCodeGenerator::testDynamicDataWidths() {
  ...
  for (uint32_t w : widths) {
    ...
    mappings.push_back(
        {"templates/uvm/reg_model.sv.inja", uvmOut.toStdString()});

    GenerationReport rep = cg.generate(
        root, "templates", QString("work/test_width_%1").arg(w).toStdString(),
        mappings);
    QVERIFY(!rep.has_errors());
...
void TestCodeGenerator::testHwPrecedencePrecedenceParam() {
  ...
  // Case 1: PARAM_HW_PRECEDENCE default or hw_precedence = true
  {
    ...
    std::vector<TemplateMapping> mappings;
    mappings.push_back(
        {"templates/rtl/reg_map.sv.inja", "work/hw_prec_1/reg_map.sv"});
    mappings.push_back(
        {"templates/rtl/reg_map.v.inja", "work/hw_prec_1/reg_map.v"});
    mappings.push_back(
        {"templates/rtl/reg_map.vhd.inja", "work/hw_prec_1/reg_map.vhd"});

    GenerationReport rep =
        cg.generate(root, "templates", "work/hw_prec_1", mappings);
    QVERIFY(!rep.has_errors());
  ...
  // Case 2: param_hw_precedence = 0 (SW precedence)
  {
    ...
    std::vector<TemplateMapping> mappings;
    mappings.push_back(
        {"templates/rtl/reg_map.sv.inja", "work/hw_prec_0/reg_map.sv"});

    GenerationReport rep =
        cg.generate(root, "templates", "work/hw_prec_0", mappings);
    QVERIFY(!rep.has_errors());
...
void TestCodeGenerator::testAllAccessPoliciesCodegen() {
  ...
  std::vector<TemplateMapping> mappings;
  mappings.push_back(
      {"templates/rtl/reg_map.sv.inja", "work/policies_test/rtl/reg_map.sv"});
  mappings.push_back({"templates/uvm/reg_model.sv.inja",
                      "work/policies_test/uvm/reg_model.sv"});

  GenerationReport rep =
      cg.generate(root, "templates", "work/policies_test", mappings);
  QVERIFY(!rep.has_errors());
  ...
  for (const auto &pol : policies) {
    QVERIFY2(uvm.contains(QString::fromStdString("\"" + pol + "\"")),
             pol.c_str());
  }
