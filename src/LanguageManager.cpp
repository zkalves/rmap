...
bool JsonTranslator::loadData(const QByteArray &jsonData) {
  ...
  // 2. Context-specific translations: { "contexts": { "context": { "src":
  // "trans" } } }
  ...
  for (auto ctxIt = contextsObj.begin(); ctxIt != contextsObj.end(); ++ctxIt) {
    ...
    for (auto it = ctxTrans.begin(); it != ctxTrans.end(); ++it) {
      m_contextMap.insert(ctxName + QChar(0x04) + it.key(),
                          it.value().toString());
    }
  }
...
QString JsonTranslator::translate(const char *context, const char *sourceText,
  ...
  if (src.contains(QLatin1Char('&'))) {
    ...
    // Or if dictionary has '&' version (e.g. key is "&File" and query is
    // "File")
...
LanguageManager::LanguageManager() { initLanguages(); }
...
void LanguageManager::initLanguages() {
  ...
  // Dynamically discover all translation catalogs from Qt resources
  // (:/translations/*.json) and fallback to translations directory on disk
  auto scanDir = [this](const QString &dirPath) {
    QDir dir(dirPath);
    if (!dir.exists())
      return;
...
bool LanguageManager::hasLanguage(const QString &codeOrName) const {
  ...
  for (const auto &l : m_languages) {
    if (l.code.toLower() == target || l.name.toLower() == target ||
        l.nativeName.toLower() == target) {
      return true;
    }
  }
  return false;
...
bool LanguageManager::setLanguage(const QString &codeOrName) {
  ...
  QString code =
      (foundIdx >= 0) ? m_languages[foundIdx].code : QStringLiteral("en");
  ...
  // Fallback: check alternate resource or filesystem path if running in dev
  // environment
  ...
  if (!jt->loadResource(resPath)) {
    delete jt;
    qWarning() << "LanguageManager: Failed to load translation resource:"
               << resPath;
    return false;
  }
