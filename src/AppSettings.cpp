...
QString AppSettings::determineConfigPath(const char *envConfig,
...
AppSettings::AppSettings() {
  m_configPath = determineConfigPath(
      std::getenv("RMAP_CONFIG_FILE"), std::getenv("XDG_CONFIG_HOME"),
      QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation));

  load();
}

QString AppSettings::configFilePath() const { return m_configPath; }
...
QString AppSettings::colorScheme() const {
  if (m_colorScheme.isEmpty() || m_colorScheme == "default") {
...
void AppSettings::setColorScheme(const QString &scheme) {
  ...
  if (s.isEmpty() || s == "default")
    s = ColorScheme::createDefault("").id;
  if (m_colorScheme != s) {
    m_colorScheme = s;
    save();
...
bool AppSettings::colorBlindMode() const { return m_colorBlindMode; }
...
ColorBlindMode AppSettings::colorBlindType() const { return m_colorBlindType; }
...
QPoint AppSettings::mainWindowPos() const { return m_mainWindowPos; }
...
QSize AppSettings::mainWindowSize() const { return m_mainWindowSize; }
...
QByteArray AppSettings::mainWindowState() const { return m_mainWindowState; }
...
QPoint AppSettings::configWindowPos() const { return m_configWindowPos; }
...
QSize AppSettings::configWindowSize() const { return m_configWindowSize; }
...
void AppSettings::setWindowGeometry(const QString &windowName,
...
QPoint AppSettings::windowPos(const QString &windowName,
                              const QPoint &defaultPos) const {
  QSettings settings(m_configPath, QSettings::IniFormat);
...
QSize AppSettings::windowSize(const QString &windowName,
                              const QSize &defaultSize) const {
  QSettings settings(m_configPath, QSettings::IniFormat);
  int w = settings
              .value(QString("Geometry/%1Width").arg(windowName),
                     defaultSize.width())
              .toInt();
  int h = settings
              .value(QString("Geometry/%1Height").arg(windowName),
                     defaultSize.height())
              .toInt();
  if (w > 0 && h > 0) {
    return QSize(w, h);
...
void AppSettings::setWindowSize(const QString &windowName, const QSize &size) {
  if (size.isValid() && size.width() > 0 && size.height() > 0) {
    QSettings settings(m_configPath, QSettings::IniFormat);
    settings.setValue(QString("Geometry/%1Width").arg(windowName),
                      size.width());
    settings.setValue(QString("Geometry/%1Height").arg(windowName),
                      size.height());
    settings.sync();
  }
}

void AppSettings::ensureWindowOnScreen(QWidget *widget, const QSize &minSize,
  ...
  if (curW < effectiveMinW) {
    curW = (defaultSize.width() >= effectiveMinW) ? defaultSize.width()
                                                  : effectiveMinW;
  }
  int effectiveMinH = (minH > 0) ? minH : 1;
  if (curH < effectiveMinH) {
    curH = (defaultSize.height() >= effectiveMinH) ? defaultSize.height()
                                                   : effectiveMinH;
  }
...
void AppSettings::load() {
  ...
  QSettings settings(m_configPath, QSettings::IniFormat);
  m_colorScheme =
      settings.value("Appearance/ColorScheme", "default").toString();
  if (m_colorScheme.isEmpty()) {
    m_colorScheme = "default";
  }
  m_colorBlindMode =
      settings.value("Appearance/ColorBlindMode", false).toBool();
  QString cbType =
      settings.value("Appearance/ColorBlindType", "universal").toString();
  ...
  m_mainWindowSplitter =
      settings.value("Geometry/MainWindowSplitter").toByteArray();
  if (settings.contains("Geometry/MainWindowX") &&
      settings.contains("Geometry/MainWindowY")) {
  ...
  m_configWindowGeometry =
      settings.value("Geometry/ConfigWindow").toByteArray();
  if (settings.contains("Geometry/ConfigWindowX") &&
      settings.contains("Geometry/ConfigWindowY")) {
    m_configWindowPos =
        QPoint(settings.value("Geometry/ConfigWindowX").toInt(),
...
void AppSettings::save() {
  ...
  if (m_configWindowSize.width() > 0 && m_configWindowSize.height() > 0) {
    ...
    settings.setValue("Geometry/ConfigWindowHeight",
                      m_configWindowSize.height());
  }
  settings.sync();
}
