...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "-t" || arg == "--theme" || arg == "--colour-scheme" ||
               arg == "--color-scheme") {
      if (i + 1 < argc)
        theme = argv[++i];
  ...
  if (show_help || argc == 1) {
    std::cout << "Usage: rmap [options] [file]\n"
              << "rmap — Hardware Register Map Designer & Model Generator "
                 "(CLI-only build)\n\n"
              << "Options:\n"
              << "  -h, --help, --help-all     Displays help on commandline "
                 "options.\n"
              << "  -v, --version              Displays version information.\n"
              << "  -f, --file <file>          Register Map file to load.\n"
              << "  -e, --export               Run in headless mode and export "
                 "templates.\n"
              << "  -o, --out <path>           Override default output "
                 "directory or output report file.\n"
              << "  -c, --convert <out_file>   Convert loaded register map to "
                 "specified output file.\n"
              << "  -l, --lint                 Run headless linter validation "
                 "on the register map file.\n"
              << "  --strict                   Enable strict validation rules "
                 "(check empty descriptions and alignment).\n"
              << "  --report-format <format>   Report format for linting "
                 "(text, json, sarif, junit) or diff (text, markdown).\n"
              << "  -d, --diff <compare_file>  Compare loaded register map "
                 "against another file.\n"
              << "  -t, --theme, --colour-scheme, --color-scheme <scheme>\n"
              << "                             Set active colour scheme.\n"
              << "  --lang, --language <lang>  Set application language.\n";
    return 0;
  }

  std::cout << "rmap " << version << " (Headless CLI mode)\n";
  if (do_lint || do_export || !convert_out.empty() || !diff_file.empty()) {
    if (file.empty()) {
      std::cerr << "Error: No register map file specified. Use -f <file> or "
                   "pass [file].\n";
      return 1;
    }
    std::ifstream fcheck(file);
    if (!fcheck.good()) {
      std::cerr << "Error: Cannot open register map file: " << file << "\n";
      return 1;
    }
  }
  if (!diff_file.empty()) {
    std::ifstream dcheck(diff_file);
    if (!dcheck.good()) {
      std::cerr << "Error: Cannot open compare file: " << diff_file << "\n";
      return 1;
    }
  }
  if (!file.empty()) {
  ...
  if (do_lint) {
    std::cout << "Running headless linter" << (do_strict ? " (strict)" : "")
              << " on " << file << "...\n";
  ...
  if (!diff_file.empty()) {
    std::cout << "Comparing " << file << " against " << diff_file << " ("
              << report_format << ")...\n";
