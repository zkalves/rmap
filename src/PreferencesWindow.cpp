...
PreferencesWindow::PreferencesWindow(QWidget *parent)
  ...
  connect(this->colourBlindMode, &QCheckBox::toggled, this->colourBlindCombo,
  ...
  connect(&ThemeManager::instance(), &ThemeManager::themesUpdated, this,
          &PreferencesWindow::populateThemes);

  QPushButton *applyBtn = this->buttonBox->button(QDialogButtonBox::Apply);
  if (applyBtn) { // GCOV_EXCL_BR_LINE - applyBtn is guaranteed by
                  // preferences.ui
...
void PreferencesWindow::saveStateFromUi() {
  ...
  if (m_colourScheme.isEmpty() || m_colourScheme == "default") {
    m_colourScheme = ColorScheme::createDefault("").id;
  }
...
void PreferencesWindow::updateUiFromState() {
  ...
  int cbIdx = this->colourBlindCombo->findData(
      colorBlindModeToString(m_colourBlindType));
  if (cbIdx >= 0) {
...
QString PreferencesWindow::language() const { return m_language; }
...
void PreferencesWindow::setColourScheme(const QString &scheme) {
  m_colourScheme = (scheme.isEmpty() || scheme == "default")
                       ? ColorScheme::createDefault("").id
                       : scheme;
  updateUiFromState();
}
...
bool PreferencesWindow::isColourBlindMode() const { return m_colourBlindMode; }

void PreferencesWindow::setColourBlindType(ColorBlindMode mode) {
  m_colourBlindType =
      (mode == ColorBlindMode::None) ? ColorBlindMode::Universal : mode;
  updateUiFromState();
}
...
void PreferencesWindow::restoreWindowStateFromSettings() {
  ...
  if (!geom.isEmpty()) {
    ...
    QSize sz = AppSettings::instance().windowSize("PreferencesWindow",
                                                  QSize(500, 280));
...
void PreferencesWindow::saveWindowStateToSettings() {
  AppSettings::instance().setWindowGeometry("PreferencesWindow",
                                            saveGeometry());
